(module
  ...
  (func $wrap (param $low i32) (param $high i32) (param $value i32) (param $shiftKey i32) (result i32)
      (local $newValue i32) (local.set $newValue (local.get $value))
      ...
      (if 
        ...
        (then 
          (if (i32.le_u (local.get $value) (local.get $high))
            (then
              (local.set
                $newValue
                (i32.add 
              ...
              (if (i32.gt_u (local.get $newValue) (local.get $high))
                (then
                  (local.set $newValue
                    (i32.sub (local.get $newValue) (i32.const 26))
                  )
                )
              )
            )
  ...
  (func (export "rotate") (param $textOffset i32) (param $textLength i32) (param $shiftKey i32) (result i32 i32)
    ...
    (loop $process
      ...
      (br_if $process (i32.lt_u (local.get $index) (local.get $stop)))
    )
